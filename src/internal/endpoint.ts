...
const ENDPOINT_RE = /^((?:background$)|devtools|popup|options|side-panel|content-script|window)(?:@(\d+)(?:\.(\d+))?)?$/;
...
export const formatEndpoint = ({ context, tabId, frameId }: Endpoint): string => {
   if (['background', 'popup', 'options', 'side-panel'].includes(context)) return context;
